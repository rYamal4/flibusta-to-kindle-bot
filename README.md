# Flibusta to Kindle Bot

Простой Telegram бот для поиска книг на Flibusta и отправки их на Kindle через email.

<p>
  <img src="images/img.png" width="400" />
</p>

<p>
  <img src="images/img_1.png" width="400" />
</p>

## Возможности

- 🔍 Поиск книг по названию и просмотр результатов
- 📖 Просмотр подробной информации о книге (автор, количество страниц, аннотация)
- 📧 Автоматическая отправка книг на Kindle через email
- 🔒 Режим доступа - публичный или только для одного пользователя
- 🐳 Готовые Docker и Docker Compose файлы

## Требования

- Java 21 или выше
- Telegram Bot Token
- SMTP сервер для отправки писем (Gmail, Yandex, Mail.ru и др.)
- Аккаунт на Amazon Kindle

Для работы бота необходимы следующие переменные среды:

```properties
TELEGRAM_BOT_TOKEN=your_bot_token_here
TELEGRAM_USER_ID=your_telegram_user_id        # только если IS_PUBLIC_BOT=false
SMTP_HOST=smtp.gmail.com:587
SENDER_EMAIL=your_email@gmail.com
SENDER_PASSWORD=your_app_password
FLIBUSTA_URL=https://m.flibusta.is
IS_PUBLIC_BOT=true                            # опционально, по-умолчанию: true
```

## Быстрый старт с Docker

1. Клонируйте репозиторий:

    ```bash
    git clone https://github.com/rYamal4/flibusta-to-kindle-bot.git
    cd flibusta-to-kindle-bot
    ```

2. Создайте файл `.env` из примера:

3. Заполните `.env` своими данными:

4. Запустите бота:

    ```bash
    docker-compose up -d
    ```

## Настройка переменных окружения

### TELEGRAM_BOT_TOKEN

Получите токен от [@BotFather](https://t.me/BotFather):

1. Напишите `/newbot`
2. Следуйте инструкциям
3. Скопируйте токен

### TELEGRAM_USER_ID

Узнайте свой ID через [@userinfobot](https://t.me/userinfobot):

1. Напишите боту `/start`
2. Скопируйте ваш ID

### SMTP настройки

#### Gmail

- SMTP_HOST: `smtp.gmail.com:587`
- SENDER_PASSWORD: используйте [App Password](https://myaccount.google.com/apppasswords)

#### Yandex

- SMTP_HOST: `smtp.yandex.ru:587`
- SENDER_PASSWORD: обычный пароль от почты

#### Mail.ru

- SMTP_HOST: `smtp.mail.ru:587`
- SENDER_PASSWORD: обычный пароль от почты

### IS_PUBLIC_BOT

Режим доступа к боту:

- `true` (по умолчанию) - бот доступен для всех пользователей
- `false` - бот доступен только для пользователя с ID, указанным в `TELEGRAM_USER_ID`

### Добавление email на Kindle

**Важно:** Добавьте `SENDER_EMAIL` в список одобренных адресов:

1. Зайдите на [Amazon Content and Devices](https://www.amazon.com/hz/mycd/myx)
2. Preferences → Personal Document Settings
3. Добавьте ваш email в "Approved Personal Document E-mail List"

После этого используйте команду `/email your@kindle.com` в боте для сохранения почты.

## Использование

### Команды

- `/start` - Справка по боту и информация о текущем email
- `/email your@kindle.com` - Установить почту вашего аккаунта Kindle

### Поиск книги

1. Просто напишите название книги боту без команды - бот автоматически начнёт поиск.
2. В результатах поиска нажмите на название интересующей вас книги, чтобы увидеть полную информацию (описание, автор,
   количество страниц).
3. На странице информации есть кнопка **"На Kindle"** для отправки книги на ваш аккаунт.

Вы также можете отправить любой файл (PDF, EPUB, DOCX, и др.) боту, и он автоматически отправит его на ваш Kindle!